...
class TextureManager {
    ...
    createGrassTexture() {
        ...
        gradient.addColorStop(0, '#2d7d2d');
        gradient.addColorStop(0.5, '#3a9a3a');
        gradient.addColorStop(1, '#2d7d2d');
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, 512, 512);
        ...
        for (let i = 0; i < 3000; i++) {
            ...
            ctx.beginPath();
            ctx.moveTo(x, y);
            ctx.quadraticCurveTo(x + angle * 10, y - length / 2, x + angle * 5, y - length);
            ctx.stroke();
        }
